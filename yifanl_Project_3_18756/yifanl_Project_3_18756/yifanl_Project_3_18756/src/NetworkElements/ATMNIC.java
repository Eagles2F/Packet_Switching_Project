...
public class ATMNIC {
	...
	private boolean drop_packet = false;
	private ArrayList<ATMCell> packetBuffer = new ArrayList<ATMCell>();
	private int current_packet_size = 0;//the number of data cells in the packet
	/**
	...
	private void runRED(ATMCell cell){
		...
		if(outputBuffer.size() > this.startDropAt){
			if(outputBuffer.size()>this.maximumBufferCells){
				...
				dropProbability = (double)(outputBuffer.size() - this.startDropAt)/(double)(this.maximumBufferCells-this.startDropAt);
			}
		}
	...
	private void runPPD(ATMCell cell){
		...
		if(this.drop_packet == false){ 	
			if(Math.random() < dropProbability){
				// drop the subsequent cells until new packet comes in
				this.drop_packet = true;
				cellDropped = true;
			}else{
				outputBuffer.add(cell);
			}
		}else{// drop the subsequent cells until new packet comes in
			if(cell.getPacketData() != null){//meaning new packet comes in
	...
	private void runEPD(ATMCell cell){
		if(cell.getPacketData() !=null){ //a header cell comes in
				//caculate the number of data cells in the packet
				this.current_packet_size = (int)Math.ceil((double)(cell.getPacketData().getSize())/(48.0*8.0)); 
			
				//get inside the packetBuffer
				this.packetBuffer.add(cell);
				//run through the RED method
				double dropProbability = 0.0;
				int len = outputBuffer.size()+packetBuffer.size();
				if(len > this.startDropAt){
					if(len>this.maximumBufferCells){
						dropProbability = 1.0;
					}else{
						dropProbability = (double)(len - this.startDropAt)/(double)(this.maximumBufferCells-this.startDropAt);
					}
				}
				//decide whether to drop this cell or not
				if(Math.random() < dropProbability){
				// drop the subsequent cells until new packet comes in
					this.drop_packet = true;
				}
		}else{//if it is not header cell
			//get inside the packetBuffer
			this.packetBuffer.add(cell);
			//run through the RED method
			double dropProbability = 0.0;
			...
			if(this.current_packet_size == this.packetBuffer.size()){// if all the cells from one packet has entered the packetBuffer
				if(this.drop_packet == true){
					for(ATMCell c:packetBuffer){
						System.out.println("The cell " + c.getTraceID() + " was dropped");
					}
					this.packetBuffer.clear();
					this.drop_packet = false;
				}else{
					for(ATMCell c:packetBuffer){
						this.outputBuffer.add(c);
						System.out.println("The cell " + c.getTraceID() + " was added to the output queue");
					}
					this.packetBuffer.clear();
				}
			}	
