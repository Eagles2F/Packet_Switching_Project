...
public class Computer implements IATMCellConsumer{
	...
	public void receiveCell(ATMCell cell, ATMNIC nic){
		if(cell.getIsOAM()){
			if(cell.getData().startsWith("call processing")){
				this.receivedCallProceeding(cell);
			}else if(cell.getData().startsWith("connect")){
				this.receivedConnect(cell);
				System.out.println("The connection is setup on VC "+this.getIntFromEndOfString(cell.getData()));
				ATMCell callack = new ATMCell(cell.getVC(),"connect ack",this.getTraceID());
				this.vcNumber = this.getIntFromEndOfString(cell.getData());
				callack.setIsOAM(true);
				this.sentConnectAck(callack);
				nic.sendCell(callack, this);
