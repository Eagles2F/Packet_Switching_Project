...
public class LSRNIC {
	...
	private void runRED(Packet currentPacket){
		...
		if(outputBuffer.size() > this.startDropAt){
			if(outputBuffer.size()>this.maximumBuffer){
				...
				dropProbability = (double)(outputBuffer.size() - this.startDropAt)/(double)(this.maximumBuffer-this.startDropAt);
			}
		}
		...
		if(packetDropped)
			System.out.println("The packet was dropped with probability " + dropProbability);
	}
	...
	public void setLink(OtoOLink link) {
		this.link = link;
	}
